...
轨道相机 该相机可围绕前方的目标位置进行旋转（就像卫星绕行地球）。可通过鼠标或触摸事件控制。该相机就像围绕地球旋转的卫星一样，它与目标点的相对位置可通过3个参数控制： alpha (角度) 纬线上的旋转, beta (角度) 经线上的旋转，以及半径 radius 与目标点的距离.
...
立体轨道相机 该相机用于模拟立体渲染 (基于ArcRotateCamera)
