...
export class WelcomeDialog extends LitElement {
  ...
  render() {
    ...
    const items = [
      {
        shotcut: 'Ctrl + O',
        describe: this.lang === 'zh' ? '打开项目' : "Open Project"
      },
      {
        shotcut: 'Ctrl + P',
        describe: this.lang === 'zh' ? '发布项目' : "Publish Project"
      },
      {
        shotcut: 'Ctrl + Shift + F',
        describe: this.lang === 'zh' ? '查找拼图' : "Search Block"
      },
      {
        shotcut: 'Alt + Shift + R',
        describe: this.lang === 'zh' ? '浏览文件夹' : "Reveal in File Explorer"
      },
    ]
    return html`
        ...
        <div class="content">
          ${items.map(item => html`
            <div class="content-row d-flex-row">
              <div class="row-left">${item.describe}</div>
              <div class="row-right">${item.shotcut}</div>
            </div>
          `)}
        </div>
